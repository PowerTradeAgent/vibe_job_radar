...
window.CollectionGuide = class {
  constructor({api, note, act, chosen, setChecks, platforms, keyConfigured}) {
    ...
    this.fieldModes = {
      ...
      search_budget: ["search"], pages: ["search"], detail_budget: ["urls", "search"],
      feed_budget: ["feed"], fresh_hours: ["urls", "search"], search_storage_rights: ["search"]
    };
  ...
  sync(locked = this.locked) {
    ...
    for (const i of document.querySelectorAll("#collect-roles input, #collect-platforms input")) i.disabled = locked;
  ...
  preset(mode) {
    ...
    this.setChecks("collect-roles", ["time_series"]);
    this.setChecks("collect-platforms", ["boss"]);
    this.setChecks("collect-permits", []);
    Object.assign(f.search_budget, {value: "1"}); f.pages.value = "1";
    f.detail_budget.value = mode === "search" ? "0" : "1";
    ...
    this.note(mode === "urls" ? "已套用URL入门参数。接下来只需粘贴1条真实职位链接、识别来源、核对许可与用途；没有填入假链接，也没有发请求。" : "已套用搜索入门参数：1岗位/1平台/1请求，只取摘要。接下来去Brave控制台取得Key并核对保存权限；未开始搜索。");
  }
  data() {
