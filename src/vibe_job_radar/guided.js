...
function render(){
 if(!state)return;
 $('environment').textContent=`当前 Python：${state.python}。Playwright：${state.browser_package||'尚未安装'}。安装状态：${state.installation}。`;
 ...
 note(sticky || (state.busy?(current?.message||'正在运行后端操作；可以暂停或停止。'):''));
}
...
$('install').addEventListener('click',()=>{if(confirm('将使用当前Python下载并安装Playwright与Chromium。不会修改你的项目文件；是否继续？'))act(()=>api('/api/guided/install',{consent:true}));});
