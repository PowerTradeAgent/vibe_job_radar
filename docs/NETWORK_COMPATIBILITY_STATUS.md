# 当前开发分支补充：Fake-IP 加密解析

PR32已合并到main@8c983bb。当前新分支在该主干之上增加网页明确授权的映射地址解析路径；仅198.18.0.0/15全映射回答触发，依旧验证公网地址/两端证书/来源许可。设置、运行路径与完整边界见[Fake-IP解析](FAKE_IP_RESOLUTION.md)。这不代表PAC/认证或所有VPN产品已经验收。下方是上一个SOCKS阶段的范围记录，不能将“Fake-IP未实现”的历史描述用于覆盖本分支新实现；实际合并与CI以对应PR为准。
